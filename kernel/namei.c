...
int vfs_path_parent_lookup(struct filename *filename, unsigned int flags,
                           ...
                           const struct path *root) {
    return __filename_parentat(AT_FDCWD, filename, flags, parent, last, type,
                               root);
}
...
static int may_open(struct mnt_idmap *idmap, const struct path *path,
    ...
    switch (inode->i_mode & S_IFMT) { // S_IFMT: 0170000 = 61440
    case S_IFLNK:                     // 0120000 = 40960
        return -ELOOP;                // 40
    case S_IFDIR:                     // 0040000 = 16384
        if (acc_mode & MAY_WRITE)     // MAY_WRITE: 2
            return -EISDIR;           // 21
        if (acc_mode & MAY_EXEC)      // MAY_EXEC: 1
            return -EACCES;           // 13
        break;
    case S_IFBLK: // 0060000 = 24576
    case S_IFCHR: // 0020000 = 8192
        if (!may_open_dev(path))
            return -EACCES;
        fallthrough;
    case S_IFIFO:  // 0010000 = 4096
    case S_IFSOCK: // 0140000 = 49152
        if (acc_mode & MAY_EXEC)
            return -EACCES;
        flag &= ~O_TRUNC; // O_TRUNC: 01000
        break;
    case S_IFREG: // 0100000 = 32768
 ...
 * raw inode simply passs @nop_mnt_idmap.
 */
static int vfs_tmpfile(struct mnt_idmap *idmap, const struct path *parentpath,
...
static struct dentry *filename_create(int dfd, struct filename *name,
    ...
    inode_lock_nested(path->dentry->d_inode, I_MUTEX_PARENT);
    dentry =
        lookup_one_qstr_excl(&last, path->dentry, reval_flag | create_flags);
    if (IS_ERR(dentry))
        goto unlock;
...
retry:
    ...
    if (!error) {
        error =
            vfs_mkdir(mnt_idmap(path.mnt), path.dentry->d_inode, dentry, mode);
    }
    done_path_create(&path, dentry);
    if (retry_estale(error, lookup_flags)) {
        lookup_flags |= LOOKUP_REVAL;
...
int vfs_rename(struct renamedata *rd) {
    ...
    if (!target) {
        ...
        if (!(flags & RENAME_EXCHANGE))
            error = may_delete(rd->new_mnt_idmap, new_dir, new_dentry, is_dir);
        else
            error =
                may_delete(rd->new_mnt_idmap, new_dir, new_dentry, new_is_dir);
    }
    if (error)
        return error;
    ...
    if (new_dir != old_dir) {
        if (is_dir) {
            error = inode_permission(rd->old_mnt_idmap, source, MAY_WRITE);
            if (error)
                return error;
        }
        if ((flags & RENAME_EXCHANGE) && new_is_dir) {
            error = inode_permission(rd->new_mnt_idmap, target, MAY_WRITE);
            if (error)
                return error;
        }
    }
...
retry_deleg:
    ...
    old_dentry = lookup_one_qstr_excl(&old_last, old_path.dentry, lookup_flags);
...
SYSCALL_DEFINE4(renameat, int, olddfd, const char __user *, oldname, int,
    ...
    return do_renameat2(olddfd, getname(oldname), newdfd, getname(newname), 0);
}

SYSCALL_DEFINE2(rename, const char __user *, oldname, const char __user *,
                newname) {
    return do_renameat2(AT_FDCWD, getname(oldname), AT_FDCWD, getname(newname),
                        0);
}
